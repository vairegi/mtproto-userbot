...
def _row_get(row: Any, name: str, default: Any = None) -> Any:
    """Named access on a libsql Row (0.3.x returns Row objects keyed by
    column name, NOT positional tuples — v1.0 indexed as r[0] and got
    `KeyError: 'result'` on every read). Tolerates dicts/tuples too."""
    try:
        return row[name]
...
async def get(key: str) -> Optional[dict]:
    ...
    try:
        ...
        if not rows:
            return None
        r = rows[0]
        payload_raw = _row_get(r, "payload")
        ...
        return {
            "payload": payload,
            "updated_at": int(_row_get(r, "updated_at", 0) or 0),
            "expires_at": int(_row_get(r, "expires_at", 0) or 0),
        }
    except Exception as e:  # noqa: BLE001
