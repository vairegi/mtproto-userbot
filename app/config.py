...
class Settings:
    ...
    # v1.20: which sort orderings to warm PER TAG. Previously tags were
    # only warmed under 'popular', so the mini-app's Popular Today / Popular
    # Week / New Uploads chips fell through to nhentai for every typed or
    # tag query. Defaults to the same 4 chip sorts users see; overridable
    # via env if the operator wants to trade warm coverage for /search
    # quota (each extra sort per tag = 1 extra warm page per tick).
    tag_sorts: List[str] = field(default_factory=lambda: _env_csv(
        "TAG_SORTS", ["popular", "popular-today", "popular-week", "date"]))
