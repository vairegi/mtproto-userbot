...
async def fetch_search_page(
    ...
    *,
    ...
    """One listing call. Always uses /api/v2/search so `sort` is honored.

    v1.20 (SORT BUG FIX): previously the empty-query path called
    /api/v2/galleries?sort=<X>&page=<N>. An isolated 2026-08-20 probe
    against live nhentai proved /galleries SILENTLY IGNORES `sort` — all
    4 sorts (popular / popular-today / popular-week / date) returned the
    byte-identical top-of-list. That is why every search:<sort>:page<N>
    row in Turso held the same payload across all 4 chip families, and
    the mini-app's chips showed identical items regardless of which sort
    was tapped. In the same probe, /api/v2/search?query=english&sort=<X>
    returned 4 completely distinct top-10s, so /search DOES honor sort.

    Fix: route empty-query chip sweeps through /search with the same
    `english` wildcard BOT 0's _direct_nhentai_search fallback already
    uses. Non-empty (typed / tag) queries keep the same /search path
    they already used.
    Returns the raw JSON blob so we cache upstream verbatim."""
    q = (query or "").strip()
    params: Dict[str, Any] = {
        "query": q or "english",   # v1.20: `english` wildcard for chip sweeps
