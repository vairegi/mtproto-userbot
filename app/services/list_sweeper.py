...
async def _fetch_and_cache(
    ...
    """
    # `sort` may be a raw sort id ('popular') OR a tag pseudo-sort. Two tag
    # forms are supported after v1.20:
    #   'tag:<name>'            legacy — warms sort=popular only (back-compat)
    #   'tag:<name>|<realsort>' v1.20 — warms <name> under the given sort,
    #                                   where <realsort> is one of
    #                                   popular / popular-today / popular-week / date.
    ...
    if is_tag:
        tag_spec = sort[4:].strip()
        # v1.20: parse '<name>|<realsort>' — 4-sort-per-tag warm.
        if "|" in tag_spec:
            tag_name, tag_sort = tag_spec.split("|", 1)
            tag_name = tag_name.strip()
            tag_sort = (tag_sort.strip() or "popular")
        else:
            tag_name = tag_spec
            tag_sort = "popular"    # legacy single-sort per tag
        ...
        # name in the mini-app hits this pre-warmed row — now sort-aware.
        key = cache.bot0_search_key(tag_name, tag_sort, page)
    else:
        # v1.20: /search with the `english` wildcard so `sort` is honored.
        # See hf_scraper_lite.fetch_search_page docstring for the probe
        # that proved /galleries silently ignored `sort`.
        query = ""
        real_sort = sort
...
async def sweep_once() -> Dict[str, Any]:
    ...
    # v1.20: warm each tag under EVERY sort listed in settings.tag_sorts
    # (default: the same 4 chip sorts users see). Previously tags were only
    # warmed under sort=popular, so tapping Popular Now / Popular Week / New
    # while inside a tag search dropped straight to nhentai (CACHE MISS →
    # user waits + eats /search quota). Encoded as 'tag:<name>|<sort>' so
    # _fetch_and_cache can parse it back out; the legacy 'tag:<name>' form
    # is still accepted (= sort=popular).
    _tag_sorts = list(settings.tag_sorts) or ["popular"]
    for slug in trending:
        for tsort in _tag_sorts:
            entry = f"tag:{slug}|{tsort}"
    ...
    for t in settings.extra_tag_sorts:
        ...
        for tsort in _tag_sorts:
            ...
            if entry not in sorts_this_phase:
                sorts_this_phase.append(entry)
    log.info("phase sort plan: %d entries (core=%d, trending=%d×%d sorts, manual=%d×%d sorts)",
             len(sorts_this_phase), len(settings.list_sorts),
             len(trending), len(_tag_sorts),
             len(settings.extra_tag_sorts), len(_tag_sorts))
