...
# ---- v12.51: admin shim ----------------------------------------------------
# routes/profile.py calls db.is_admin_user(uid). If admins are tracked in a
# collection, this reads it; otherwise falls back to False (non-admin).
# Wired lazily so it never breaks boot if the collection is missing.
def is_admin_user(user_id) -> bool:
    try:
        col = globals().get("col_admins")
        if callable(col):
            doc = col().find_one({"user_id": int(user_id)})
            return bool(doc)
    except Exception:
        pass
    return False
