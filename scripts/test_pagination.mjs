...
const pageButtons = (root) => {
  const bar = root.querySelector(".du-pagination");
  return bar ? bar.querySelectorAll(".du-page-btn") : [];
};
const btnByLabel = (root, label) =>
  pageButtons(root).find(b => b.textContent.trim() === label);
...
console.log("\nTEST 1 — pages 1→4: grid + state hold ONLY the current page");
{
  ...
  check("active page is 1", activeBtn(root)?.textContent.trim() === "1");
  ...
  check("page 4 cards are page-4 items (4000-series)", ids.every(id => id.startsWith("4")),
        `first=${ids[0]}`);
  check("active page is 4", activeBtn(root)?.textContent.trim() === "4");
  ...
  check("› disabled at known end (has_more=false)", btnByLabel(root, "›").disabled === true);
  ...
  await new Promise(r => setTimeout(r, 0)); await new Promise(r => setTimeout(r, 0));
  check("‹ from 5 lands on 4", activeBtn(root3)?.textContent.trim() === "4");
  ...
  btnByLabel(root3, "»").click();
  await new Promise(r => setTimeout(r, 0)); await new Promise(r => setTimeout(r, 0));
  check("» jumps to the learned last page (5)", activeBtn(root3)?.textContent.trim() === "5");
  check("» target fetched page 5", lastSearchCall()?.page === 5);

  // « jumps to first
  btnByLabel(root3, "«").click();
  await new Promise(r => setTimeout(r, 0)); await new Promise(r => setTimeout(r, 0));
  check("« lands on page 1", activeBtn(root3)?.textContent.trim() === "1");
  ...
  await new Promise(r => setTimeout(r, 0)); await new Promise(r => setTimeout(r, 0));
  check("on page 2 before chip tap", activeBtn(root)?.textContent.trim() === "2");
  ...
  check("active page back to 1", activeBtn(root)?.textContent.trim() === "1");
}
...
console.log("\nTEST 5 — mount with #search?page=2&sort=popular-week restores page 2");
{
  ...
  check("active page is 2", activeBtn(root2)?.textContent.trim() === "2");
